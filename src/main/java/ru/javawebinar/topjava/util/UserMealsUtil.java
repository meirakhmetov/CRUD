...
public class UserMealsUtil {
    public static void main(String[] args) {
        List<UserMeal> meals = Arrays.asList(
                ...
                new UserMeal(LocalDateTime.of(2020, Month.JANUARY, 30, 20, 0), "Ужин", 501),
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for(UserMeal meal:meals){
            if(new TimeUtil().isBetweenInclusive(meal.getDateTime().toLocalTime(),startTime,endTime)){
                list.add(new UserMealWithExcess(meal.getDateTime(),
                        meal.getDescription(),
                        meal.getCalories(),
                        new ExcessUtil().isExcess(meals,meal.getDateTime().toLocalDate(),caloriesPerDay)));
            }
